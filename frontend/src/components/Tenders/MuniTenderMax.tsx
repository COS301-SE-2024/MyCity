  ...
  const getDialogText = (action: string) => {
    switch (action) {
      case "Mark as Complete":
        return "Are you sure you want to close this tender contract?";
            ...
            <div className="relative w-full lg:w-1/3 p-2 flex flex-col items-center">
              ...
              <div
                className={`px-2 py-1 rounded-full text-sm border-2 mb-2 ${
                  statusStyles[getStatus(tender.status)]
                }`}
              >
                {tenderStatus}
              </div>
                ...
                {tender.status === "completed" && (
                  <>
                    <button
                      ...
                      onClick={() => handleAction("Terminate Contract")}
                    >
                      Terminate
                    </button>
                    <button
                      className="bg-blue-500 text-white text-sm rounded-lg px-2 hover:bg-blue-600"
                      onClick={() => handleAction("Mark as Complete")}
                    >
                      Mark Complete
                    </button>
                  </>
          ...
          <div className="bg-white p-4 rounded shadow-lg text-black">
            ...
            <div className="flex justify-center gap-2">
              <button
                ...
                onClick={() => setDialog({ action: "", show: false })}
              >
                Cancel
              </button>
