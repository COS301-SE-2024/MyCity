...
export default function TopAssetsProgress({ data }: TopAssetsProgressProps) {
  ...
  const estimatedTimes = JSON.parse(data.by_asset_by_etc_time.replace(/'/g, '"'));
  ...
  const sortedAssets = Object.entries(faultReports)
    .filter(([asset]) => avgTimes[asset] !== undefined && estimatedTimes[asset] !== undefined)
    .sort(([assetA], [assetB]) => {
  ...
  return (
    <div className="bg-white rounded-lg p-4 shadow-lg h-[40vh] w-full overflow-auto">
      <h3 className="text-black text-2xl mb-2">Asset Resolution Times</h3>
      <div className="space-y-4">
        {paginatedAssets.map(([asset]) => {
          const avgTime = avgTimes[asset];
          const estimatedTime = estimatedTimes[asset];
          const avgTimePercentage = (avgTime / Math.max(avgTime, estimatedTime)) * 100;
          const estimatedTimePercentage = (estimatedTime / Math.max(avgTime, estimatedTime)) * 100;
          const avgTimeBarColor = avgTime < estimatedTime ? 'rgba(144, 238, 144, 0.6)' : 'rgba(255, 99, 132, 0.6)';
            ...
            <div key={asset}>
              ...
              <div className="w-full bg-gray-200 rounded-full h-6 mb-2 relative">
                <div
                  ...
                  style={{ width: `${avgTimePercentage}%`, backgroundColor: avgTimeBarColor }}
                ></div>
              ...
              <div className="w-full bg-gray-200 rounded-full h-6 relative">
                <div
                  className="h-6 rounded-full"
                  style={{ width: `${estimatedTimePercentage}%`, backgroundColor: 'rgba(173, 216, 230, 0.6)' }}
                ></div>
                <span className="absolute left-2 top-0 text-xs font-bold h-full flex items-center">
