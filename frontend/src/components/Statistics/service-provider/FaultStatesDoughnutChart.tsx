...
export default function FaultStatesDoughnutChart({
  data,
}: FaultStatesDoughnutChartProps) {
  ...
  const getState = (state: string) => {
    ...
    return (
      FaultStates[normalizedState as keyof typeof FaultStates] ||
      FaultStates.Default
    );
  };

  // Map fault states to their respective colors from FaultStates
  const backgroundColors = Object.keys(resolvedVsUnresolved).map(
    (state) => getState(state).color
  );
  ...
  return (
    <div className="bg-white bg-opacity-90 w-full  rounded-lg p-4 shadow-lg h-[38vh]">
      <h2 className="text-2xl font-bold mb-4 text-center">Contract States</h2>
      <div className="h-[80%]">
        <Doughnut
          data={donutChartData}
