...
const ServiceProvidersGrid = ({
  ...
  if (displayedProviders.length < providersPerPage) {
    const placeholders = new Array(
      providersPerPage - displayedProviders.length
    ).fill(null);
  ...
  function formatServiceProvider(mun: string): string {
    ...
    // console.log(mun.replace(/ /g, "_"));
    mun = mun.substring(0, mun.indexOf(":"));
    return mun.replace(/ /g, "_");
  }

  return (
    <div className="bg-white bg-opacity-90 rounded-lg p-4 shadow-lg h-[38vh]">
      <h2 className="text-2xl font-bold mb-2 text-center">Municipalities</h2>
      <div className="grid grid-cols-5 gap-2 h-[75%] overflow-hidden mb-2">
        {displayedProviders.map((provider, index) =>
          provider ? (
            ...
            <div key={index} className="bg-transparent h-full rounded-lg p-2" />
          )
        )}
      </div>
