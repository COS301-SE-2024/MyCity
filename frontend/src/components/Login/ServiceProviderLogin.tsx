...
export default function ServiceProviderLogin() {
  ...
  const handleSubmit = async (event: FormEvent) => {
    ...
    try {
      const { isSignedIn } = await handleSignIn(new FormData(event.currentTarget as HTMLFormElement), UserRole.PRIVATE_COMPANY);
  ...
  return (
    <div className="px-12">
      <form
        data-testid="service-provider-login-form"
