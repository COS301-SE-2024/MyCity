...
import { FaEye, FaEyeSlash, FaSpinner, FaQuestionCircle, FaInfoCircle, FaCheck, FaTimes, FaAngleUp, FaAngleDown } from 'react-icons/fa';
...
export default function ServiceProviderSignup() {
  ...
  useEffect(() => {
    const { firstname, surname, email, password, confirmPassword, authcode } = formData;
    const formValid =
      firstname &&
      surname &&
      email &&
  ...
  return (
    <div className="px-12">
      <form data-testid="service-provider-signup-form" onSubmit={handleSubmit} className="flex flex-col gap-y-4 pt-8">
        ...
        <div className="relative w-full mt-4">
          ...
          {isHelpVisible && (
            <div className="absolute top-[-3rem] right-0 bg-white border rounded-lg p-1 text-xs shadow-lg w-40">
              AuthCode is a unique identifier to verify your association with your company.
            </div>
          )}
        </div>
