...
def test_create_ticket_missing_fields(test_client):
    ...
    for case in test_cases:
        ...
        assert response.status_code == 200, f"Expected status code 200 but got {response.status_code} for data: {case}"
        response_body = json.loads(response.body)
        assert response_body["Status"] == "FAILED", f"Status should be FAILED for invalid data: {case}"
        assert "Error" in response_body, f"Response should have an Error for invalid data: {case}"
...
def test_add_comment_without_image_missing_fields(test_client):
    invalid_comment_data = [
        {},
        {"ticket_id": "58a1dadb-1f07-43b0-9869-984dd80cffd4", "user_id": "qinisela.mthembu@yahoo.com"},
        {"comment": "Test comment", "ticket_id": "", "user_id": "qinisela.mthembu@yahoo.com"},
        {"comment": "", "ticket_id": "58a1dadb-1f07-43b0-9869-984dd80cffd4", "user_id": ""}
    ]
    ...
    for data in invalid_comment_data:
        ...
        assert response.status_code == 400
        assert response.json_body.get("Message") == "Missing required field: comment, ticket_id, or user_id"
...
def test_get_ticket_comments_invalid(test_client):
    ...
    for ticket_id in invalid_ids:
        ...
        assert response.status_code == 400
        assert response.json_body.get("Message") == "Failed to search for the ticket comments"
'''
